...
function [best_q, bhat,score,gap,rtime,ncount]=cv_best_subset_maximum_score(tr_ind,test_ind,data,focus_ind,aux_ind,beta0,q_range,T,tol,bnd)
...
[bhat(:,i,q),score(i,q),gap(i,q),rtime(i,q),ncount(i,q)]  = max_score_constr_fn(y,datax(:,focus_ind),datax(:,aux_ind),beta0,q_range(q),T,tol(q),bnd);
